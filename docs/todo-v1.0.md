# ✅ VarSync v1.0 - 완료된 작업 목록

> v1.0 출시(2025-05)까지 완료된 모든 작업 기록. 현재 진행 중인 작업은 `todo.md` 참고.
...
- [x] GitHub Token을 매번 입력하지 않도록 로컬 저장 (`figma.clientStorage`)
- [x] 진행 상황 표시 (로딩 스피너)
- [x] 2단계 UI 분리 (Step 1: GitHub 설정, Step 2: 컬렉션 선택)
- [x] 생성할 브랜치명 입력 (없으면 자동 생성, 이미 존재하면 재사용)

## 🔒 보안 체크리스트

- [x] 에러 경로에서 `pendingSyncParams`(토큰 포함) 즉시 클리어 — `error` 메시지 수신 시 null 처리 누락 버그 수정
- [x] `figma.clientStorage` 토큰 저장 시 평문 저장 금지 — AES-GCM(PBKDF2 키 유도) 암호화 후 저장, 저장 전 보안 경고 표시
