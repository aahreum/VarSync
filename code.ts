...
figma.showUI(__html__, { width: 332, height: 400 });
...
(async () => {
  try {
    ...
    figma.ui.postMessage({
      type: 'error',
      message: e instanceof Error ? e.message : '컬렉션 목록을 불러오지 못했습니다.',
    });
  }
})();
...
figma.ui.onmessage = async (msg: PluginMessage) => {
  if (msg.type === 'request-variables') {
    try {
      const files = await buildTokensByCollection(msg.payload.selectedCollectionIds);
      figma.ui.postMessage({
        type: 'variables-data',
        payload: { files },
      });
